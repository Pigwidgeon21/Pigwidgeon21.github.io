...
<div class="main_column column" id="main_column">
	...
	if($query == "")
		...
		//If there is an underscore, assume search is for username
		if($type == "username") 
			$usersReturnedQuery = mysqli_query($con, "SELECT * FROM users WHERE username LIKE '$query%' AND user_closed='no' LIMIT 8");
		//If there are two words, assume order is first and last name
		else {

			$names = explode(" ", $query);

			if(count($names) == 3) //middle name included
			...
			//If one word only, search for first or last name 
		...
		while($row = mysqli_fetch_array($usersReturnedQuery)) {
			...
			if($user['username'] != $row['username']) {
				...
				//Generate button/friendship status 
